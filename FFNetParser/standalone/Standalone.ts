...
class Standalone {
            ...
            $('.navi')
                ...
                .each((k, el) => {
                    ...
                    if (element.attr('href')[0] == '/') {
                        ...
                        element.attr(
                            'href',
                            self.BasePath + element.attr('href')
                        );
    ...
    private startHashTimer() {
        ...
        this.eventHandler.addTimedTrigger(
            'standaloneHashTimer',
            'standaloneOnHashTimerTick',
            1000,
            this,
            null
        );
        this.eventHandler.addEventListener(
            ...
            (s, e) => {
                if (
                    ...
                    document.location.hash !== this.lastHash
                ) {
                    this.lastHash = document.location.hash;
    ...
    public updatePage(url: string, callback: () => void) {
        ...
        this.getPageElements(this.BasePath + url, function (res) {
            $('.FFNetContentContainer').html(
                $('<div></div>').append(res.Content).html()
            );
    ...
    public getPageElements(
        url: string,
        callback: (result: { Content: JQuery; Navigation: JQuery }) => void
    ) {
    ...
    private handleFilterModal(modal: JQuery) {
        ...
        this.filterModal = GUIHandler.createBootstrapModal(
            modalBody,
            'Filter',
            buttons
        );
    ...
    public applyFilter() {
        ...
        path += this.gt_zero_append(
            $('select[name=characterid1]').val(),
            '&c1='
        );
        path += this.gt_zero_append(
            $('select[name=characterid2]').val(),
            '&c2='
        );
        path += this.gt_zero_append(
            $('select[name=characterid3]').val(),
            '&c3='
        );
        path += this.gt_zero_append(
            $('select[name=characterid4]').val(),
            '&c4='
        );
        path += this.gt_zero_append(
            $('select[name=_characterid1]').val(),
            '&_c1='
        );
        path += this.gt_zero_append(
            $('select[name=_characterid2]').val(),
            '&_c2='
        );
    ...
    public insertStyle() {
        this.getRawPageContent('ffnetStyle.css', (s) => {
            ...
            $('<style type="text/css"></style>').text(s).appendTo($('head'));
        });
    }
    ...
    public getRawPageContent(url: string, callback: (page: string) => void) {
        ...
        $.get(url, function (content) {
            self.callEvent('standaloneOnRequestDone', this, {
                url: url,
                content: content,
            });
            ...
            self.callEvent('standaloneOnRequestFail', this, {
                url: url,
                event: event,
            });
        });
        ...
        this.getCategories((result) => {
            ...
            $.each(
                ...
                function (
                    ...
                    var rowContainer = $('<div clas="row"></div>').appendTo(
                        '.categoriesContainer'
                    );

                    $(
                        '<div class="col-md-12" style="background-color: #f5f5f5"><h2>' +
                            name +
                            '</h2></div>'
                    ...
                    $.each(
                        elements,
                        function (_, element: { name: string; url: string }) {
                            var container = $(
                            ...
                            container.append(
                                $(
                                    ...
                                    .text(element.name) //  style="width:100%"
                                    .click(function (e) {
                                        e.preventDefault();

                                        document.location.href =
                                            'elements.html?cat=' + element.url;
                                    })
                            );
                        }
                    );
    ...
    public getElements(
        ...
        callback: (
            name: string,
            data: { name: string; url: string; count: string }[]
        ) => void
    ) {
        var self = this;
    ...
    public manageElements() {
        ...
        if (url !== undefined) {
            ...
            this.getElements(url, function (name, elements) {
                ...
                $(
                    '<table class="table" style="width:90%"><thead><tr><th>Name</th><th>Stories</th></tr></thead></table>'
                )
                    .append(body)
                ...
                $.each(
                    ...
                    function (
                        _,
                        element: { name: string; url: string; count: string }
                    ) {
                ...
                self.callEvent('standaloneElementsParsed', self, [
                    name,
                    elements,
                ]);
                self.progress.hide();
            ...
            $('.elementsContainer').html(
                "<h1>Please don't open this Page directly! Redirecting ..."
            );
