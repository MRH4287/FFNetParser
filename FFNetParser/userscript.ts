...
class StoryParser {
     ...
     */
    public api_getLanguage(
        languageCode: string,
        callback?: (response: LanguageData) => void,
        apply: boolean = false,
        save: boolean = true
    ) {
        if (this.DEBUG) {
        ...
        if (languageCode === this.config.language) {
            if (typeof this.dataConfig['language'] !== 'undefined') {
                ...
                this.log(
                    'No local Language Data. Requesting from Server ...',
                    this.dataConfig
                );
            }
        }
     ...
     */
    public api_getNeedUpdate(
        callback: (result: { upload: string[]; download: string[] }) => void
    ) {
