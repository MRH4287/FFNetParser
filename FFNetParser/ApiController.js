...
class ApiController extends ExtentionBaseClass {
    ...
    JsonPRequest(url, data, apiLookupKey, timeout, retries, callback) {
        var self = this;
    ...
    */
     ...
     */
    GetLanguage(languageCode, callback, apply = false, save = true) {
        if (this.DEBUG) {
    ...
    */
    GetReadChapters(storyIDs, callback) {
        var request = {
            Token: this.Config.token,
            Chapters: storyIDs
        };
        ...
        if (storyIDs.length > 5) {
            ...
            var outResult = {};
            var outLastChapter = {};
            var dataCallback = (result, lastChapter) => {
                if (this.DEBUG) {
