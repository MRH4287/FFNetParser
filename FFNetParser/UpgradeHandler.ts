...
class UpgradeHandler extends ExtentionBaseClass {
    private registeredTags: {
        [index: string]: {
            ifNotExist?: (self: UpgradeHandler) => void;
            ifExist?: (self: UpgradeHandler) => void;
        };
    } = {};
    ...
    public initTags() {
        ...
        this.registerTag('filter_keep_searching', function () {
            $.each(
                self.config.marker,
                function (name: string, data: MarkerConfig) {
        ...
        this.registerTag('filter_Image_highlighter_5.2.9', function () {
            ...
            $.each(self.config.highlighter, function (link, element) {
                ...
                if (
                    typeof self.config.highlighter[link].custom ===
                        'undefined' &&
                    typeof self.config.highlighter[link].prefab === 'undefined'
                ) {
        ...
        this.registerTag('highlighter_defaultValues', function () {
            ...
            $.each(highlighterDefault, function (name, element) {
                if (
                    typeof self.config.highlighterPrefabs[name] === 'undefined'
                ) {
    ...
    public handleTags() {
        ...
        $.each(
            ...
            function (
                name: string,
                data: {
                    ifNotExist?: (self: UpgradeHandler) => boolean;
                    ifExist?: (self: UpgradeHandler) => boolean;
                }
            ) {
                if (self.DEBUG) {
                ...
                if (
                    typeof self.config.upgradeTags[name] === 'undefined' &&
                    typeof data.ifNotExist !== 'undefined'
                ) {
                    if (self.DEBUG) {
                        console.log(
                            'Upgrade Handler: Executing IfNotExist Handler for Tag: ',
                            name
                        );
                    }
                    ...
                    typeof self.config.upgradeTags[name] !== 'undefined' &&
                    typeof data.ifExist !== 'undefined'
                ) {
                    if (self.DEBUG) {
                        console.log(
                            'Upgrade Handler: Executing IfExist Handler for Tag: ',
                            name
                        );
                    }
    ...
    private registerTag(
        ...
        if (
            typeof ifNotExist === 'undefined' &&
            typeof ifExist === 'undefined'
        ) {
            this.log(
                'Upgrade Handler: At least one of the callback Functions has to be set!',
                name
            );
            return;
        }
