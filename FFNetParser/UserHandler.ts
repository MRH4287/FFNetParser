...
class UserHandler extends ExtentionBaseClass {
    ...
    public constructor(parser: StoryParser) {
        ...
        this.parser.eventHandler.addEventListener(
            'postElementParse',
            (sender, element) => {
                    ...
                    if (this.userRegEx.test(href)) {
                        ...
                        if (res[1] !== undefined) {
                            try {
                                ...
                                if (
                                    this.specialUsers.indexOf(userNumber) !== -1
                                ) {
                                    this.doRainbow(
                                        el,
                                        '- Special Supporter of the Fanfiction Story-Parser'
                                    );
                                }
                            } catch (exception) {}
                        }
    ...
    public doRainbow(
        ...
        for (var i = 0; i < text.length; i++) {
            ...
            var color =
                this.rainbowColors[currentColor % this.rainbowColors.length];
